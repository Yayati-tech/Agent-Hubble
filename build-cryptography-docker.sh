...
RUN zip -r cryptography-package.zip python/

# Copy the package to a volume
CMD cp cryptography-package.zip /output/
DOCKERFILE_EOF
...
# Extract the cryptography modules to the deployment directory
cd crypto-output
unzip -q cryptography-package.zip
cd ..
